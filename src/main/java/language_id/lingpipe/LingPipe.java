...
public class LingPipe extends LanguageClassifier {
    ...
    @Override
    public Result detectLanguage(String text, String target_langcode) throws IOException, ClassNotFoundException {

        JointClassification classification = (JointClassification) this.classifier.classify(text);
        String pred_lang = classification.bestCategory();
        if (pred_lang != null && pred_lang.equals("swh")) {
            pred_lang = "swa";
        }
        double pred_lang_conf = classification.conditionalProbability(0);
        boolean isReliable = false;
        if (pred_lang_conf >= RELIABLE_THRESH) {
            isReliable = true;
        }
        
        double target_lang_conf = 0.0;
        if (target_langcode.equals(pred_lang)) {
            target_lang_conf = pred_lang_conf;
        } else {
            target_lang_conf = classification.conditionalProbability(target_langcode);
        }
        
        
        return new Result(pred_lang, isReliable, pred_lang_conf, target_lang_conf, "lingpipe");
    }
    ...
    public JointClassification detectLanguage2(String text) throws IOException, ClassNotFoundException {
        ...
        //return new Result(bestLang,isReliable,probability, "lingpipe");
